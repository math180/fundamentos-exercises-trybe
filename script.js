...
const biggerNumber = (bigger, otherNumber) => ((bigger > otherNumber) ? bigger : otherNumber )  
...
const onlyPair = () => numbers1.filter((number) => number % 2 === 0).reduce((first, second) => first + second)
...
const onlyReduce = () => {
   return numbers1.reduce((number1, number2) => (number2 % 2 === 0) ? number1 + number2 : number1, 0)
}
...
const bestGrade = estudantes.map((student) => ({
    name: student.nome,
    materia: student.materias.reduce((one, two) => (one.nota > two.nota) ? one : two).name
}));
